...
class Settings:
    ...
    assemblyai_min_turn_silence = int(os.getenv("ASSEMBLYAI_MIN_TURN_SILENCE_MS", "250"))
    assemblyai_max_turn_silence = int(os.getenv("ASSEMBLYAI_MAX_TURN_SILENCE_MS", "550"))
    ...
    ingress_chunk_seconds = _float("INGRESS_CHUNK_SECONDS", 0.08)
    ...
    ytdlp_player_clients = os.getenv(
        ...
        "mweb,web_safari,web_embedded,web_creator,ios,android,tv",
    )
    ytdlp_force_ipv4 = os.getenv("YTDLP_FORCE_IPV4", "true").lower() == "true"
    ytdlp_sleep_requests = _float("YTDLP_SLEEP_REQUESTS", 0.2)
